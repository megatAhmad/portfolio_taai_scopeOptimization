    ...
    <section className="theme-panel rounded-[2rem] border border-white/70 bg-white/80 p-6 shadow-panel backdrop-blur transition-colors duration-300">
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1.7fr)_minmax(320px,0.65fr)] 2xl:grid-cols-[minmax(0,1.9fr)_minmax(340px,0.55fr)]">
        <div className="theme-table-wrap overflow-x-auto rounded-[1.5rem] border border-slate-200 transition-colors duration-300">
          <table className="theme-results-table min-w-full border-collapse text-sm">
            ...
            <tbody>
              {result.rows.map((row, index) => (
                <tr key={result.offset + index} className="theme-results-row border-t border-slate-100 align-top odd:bg-white even:bg-slate-50/50">
                  <td className="px-4 py-3">
                    <button type="button" onClick={() => setSelectedExplanationIndex(index)} className="theme-table-button inline-flex items-center gap-2 rounded-full bg-slate-100 px-3 py-2 text-xs font-semibold text-slate-700">
                      <Eye size={14} /> Trace
                    </button>
                  </td>
